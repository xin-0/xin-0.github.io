...
E(U_{\Delta t}^m,e^{-2miH\Delta t})
	& = \max_{\ket\psi}\Vert{(U_{\Delta t}^m-e^{-2miH\Delta t})\ket{\psi}}\Vert \\
\end{align}$$

Since \\(H\\) obviously commute with itself,

$$\begin{align}
U_{\Delta t}^m-e^{-2miH\Delta t}
	& = \left(e^{-2iH\Delta t}+O(\Delta t^3)\right)^m-e^{-2miH\Delta t} \\
	& = C_m^1\cdot e^{-2i(m-1)H\Delta t} O(\Delta t^3)+C_m^1\cdot e^{-2i(m-2)H\Delta t} O(\Delta t^3)^2\cdots+
\end{align}$$